...
for val in $threads; do lastblock=$(docker logs $val --tail 100 | awk '/Processed block/ {block=$NF} END {print block}'); echo "Last block of $val: $lastblock" ; if [ -z $lastblock ]; then name=$(sudo docker restart $val); echo -e "${RED}Restart: $val - Not activated${NC}"; elif [ "$lastblock" -lt "$block" ]; then sudo docker restart $val >null; echo -e "${RED}Restart: $val - Missing $(($currentblock - $lastblock)) blocks${NC}"; else echo -e "${GREEN}Passed${NC}"; fi; done
